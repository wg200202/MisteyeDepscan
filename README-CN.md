...
> **重要说明：** DepScan 在上传查询时**仅会发送本地包名及对应版本号**（例如 `requests==2.32.3`、`lodash@4.17.21`）至 MistEye API，用于威胁情报匹配。
>
> **不会**采集或上传您的源代码、文件内容、项目路径、环境变量、API Key、账号凭证或其它隐私数据。依赖发现与清单解析均在您本机本地完成。
